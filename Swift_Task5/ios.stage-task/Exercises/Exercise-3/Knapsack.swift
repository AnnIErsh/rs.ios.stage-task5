...
public final class Knapsack {
    ...
    func fillDistributions(supplies: inout [[Int]], line: (Int, Int), i: inout Int, j: Int) {
        while (i < maxWeight)
        {
            if ((i - line.0 == 0 || (i - line.0 > 0 && supplies[j][i - line.0] > 0)))
            {
                supplies[j + 1][i] = max(supplies[j][i - line.0] + line.1, supplies[j][i])
            }
            else
            {
                supplies[j + 1][i] = supplies[j][i]
            }
            i += 1
        }
    }
